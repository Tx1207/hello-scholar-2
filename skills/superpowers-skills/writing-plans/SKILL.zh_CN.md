...
必需标题、字段标签、路径、命令、代码标识符、skill 名称和技术术语保持原文。

用户可读的计划正文遵循仓库语言偏好。当仓库声明默认语言时，goal、architecture notes、task descriptions、expected results 和 review notes 等正文使用该默认语言。仓库默认语言明确时，不要根据任务提示语言推断计划正文语言。
...
**"Plan complete and saved under the current task's project or worktree root at `hello-scholar/memory/plans/<filename>.md`. Two execution options:**

...
在这个 skill 中，“测试”指实现前能因正确原因失败的最便宜证据工件。对普通确定性代码，它通常是单元测试或集成测试。对 AI、agent、RAG、prompt、skill 或科研任务，它也可以是 eval case、trajectory check、benchmark smoke run、scorer 或 pressure scenario。

选择能证明目标行为的最便宜模式：

| Pattern | 适用 | 成本 |
|---------|------|------|
| `behavior-unit-test` | 确定性函数、解析器、CLI 行为 | fast |
| `contract-integration-test` | API、文件格式、工具协议 | fast/focused |
| `prompt-eval-case` | prompt、抽取、分类、改写 | fast/focused |
| `rag-eval-case` | 检索、引用、grounding | focused |
| `agent-trajectory-test` | 工具使用、多轮 agent、handoff | focused |
| `research-benchmark` | train/eval/ablation/reproduction | expensive/scheduled |
| `skill-pressure-test` | skill 或 agent 流程行为 | focused |
...
编写一个最小证据工件，展示应该发生什么。它必须足够便宜，能进入内循环，并且在实现前因预期原因失败。
...
**要求：**
- 一个行为、scenario 或 eval case
- 清晰名称
- 真实代码、真实 prompt、真实 trace 或真实 scorer（除非不可避免，否则不要 mock）
- 足够小，能按所选成本层级运行
...
- 证据失败（不是报错）
- 失败消息符合预期
- 失败是因为行为缺失（不是拼写错误、设置错误或 noisy eval）
...
| **Right-sized** | RED 很快，更强 gate 放到完成前 | 每次编辑都跑 full benchmark |
...
- [ ] 每个新行为都有测试或证据工件
- [ ] 在实现前看过每个证据工件失败
- [ ] 每个失败都因预期原因发生（行为缺失，而不是拼写/设置/noise）
- [ ] 编写了能通过所选 gate 的最小代码
- [ ] 必需 tests/evals/checks 通过
- [ ] 输出干净（无错误、警告）
- [ ] 证据使用真实代码、prompts、traces、scorers 或 commands（只有不可避免时才 mock）
- [ ] 覆盖边界情况和错误
...
测试 AI、agent、prompt、RAG、research 或 skill 行为时，阅读 `references/evidence-pattern-gallery.zh_CN.md` 来选择合适的证据模式和成本层级。
